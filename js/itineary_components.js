...
class SightSuggestions extends React.Component {
  constructor(props) {
    super(props);
  ...
  render(){
    ...
    if (typeof this.props.node.address !== 'undefined'){
      in_page_map_link = (<a className='suggestions__item' href='#' onClick={this._showOnMap}>show</a>);
...
class DaySuggestions extends React.Component {
  constructor(props) {
    super(props);
    this._showAllOnMap = this._showAllOnMap.bind(this);
  }
  _showAllOnMap(e) {
    e.preventDefault();
    let validAddresses = this.props.allAddresses.filter((x) => x !== undefined && x !== null && x !== "")
    this.props.showMultipleAddresses(validAddresses);
  }
  render(){
    return (
      <div className='suggestions'>
...
class Node extends React.Component {
  render(){
    ...
    var suggestions;
    if (typeof this.props.config['planningMode'] !== 'undefined' && node.type === 'S'){
      suggestions = (<SightSuggestions node={node} showSingleAddress={this.props.showSingleAddress}/>)
    }
    if (typeof this.props.config['planningMode'] !== 'undefined' && node.type === 'D'){
      suggestions = (<DaySuggestions node={node} showMultipleAddresses={this.props.showMultipleAddresses} allAddresses={this.props.allAddresses}/>)
    }
...
class Day extends React.Component {
  ...
  render(){
    ...
    let allAddresses = this._collectAddresses(nodes);
    
    nodes = nodes.map(function(node, index, array){
      if (node.type === "D") {
        return (
          <Node key={index} node={node} drawVertLine={(index != array.length-1)} allAddresses={allAddresses} {...this.props} />
        )
      } else {
        return (
